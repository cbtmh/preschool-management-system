...
export default function ParentProfileScreen() {
  ...
  const loadProfile = async () => {
    try {
      ...
      console.log('GetMe error:', error.response?.data || error);
      const errorMsg = error.response?.data?.message || error.message || 'Không thể tải thông tin cá nhân';
      Alert.alert('Lỗi', `Chi tiết: ${errorMsg}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const initial = profile?.fullName ? profile.fullName.charAt(0).toUpperCase() : 'P';
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.profileCard}>
          ...
          <Text style={styles.userName}>{profile?.fullName || 'Phụ huynh'}</Text>
          <Text style={styles.userRole}>{role === 'PARENT' ? 'Phụ huynh' : role}</Text>
        ...
        <View style={styles.sectionContainer}>
          <View style={styles.sectionHeader}>
            ...
            <TouchableOpacity onPress={() => navigation.navigate('EditParentProfile', { profile: userProfile })}>
